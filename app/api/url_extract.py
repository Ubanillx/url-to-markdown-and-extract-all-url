...
async def extract_static_webpage(request: UrlExtractRequest):
    ...
    - markdown_content: 网页内容的Markdown格式
    ...
    """
    try:
        result = url_extractor.extract_urls_and_markdown(request)
...
async def extract_dynamic_webpage(request: UrlExtractRequest):
    ...
    """
    try:
        result = selenium_extractor.extract_urls_and_text(request)
...
async def extract_static_webpage_get(
    ...
    """
    通过GET请求提取静态网页中的所有URL链接并转换为Markdown格式
...
async def extract_dynamic_webpage_get(
    ...
    """
    通过GET请求提取动态网页中的所有URL链接并转换为Markdown格式
...
async def extract_urls_and_markdown(request: UrlExtractRequest, use_selenium: bool = False):
    """
    通用网页提取端点 - 自动选择静态或动态提取方法
    
    - **url**: 要处理的网页URL
    - **include_external**: 是否包含外部链接 (默认: True)
    - **include_internal**: 是否包含内部链接 (默认: True)
    - **max_links**: 最大链接数量限制 (可选)
    - **use_selenium**: 是否强制使用Selenium进行JavaScript渲染 (默认: False，自动检测)
    ...
    - success: 是否成功
    - error_message: 错误信息(如果有)
    - method: 使用的提取方法 (requests/selenium)
    """
    try:
        if use_selenium:
            result = selenium_extractor.extract_urls_and_text(request)
...
@router.get("/extract")
async def extract_urls_and_markdown_get(
    url: str,
    include_external: bool = True,
    include_internal: bool = True,
    max_links: int = None,
    ...
    """
    通过GET请求提取网页中的所有URL链接并转换为Markdown格式
